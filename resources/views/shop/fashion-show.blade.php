    ...
    <div class="panel panel-default">
	    <div class="panel-body">
	        ...
	        <table class="table table-striped">
			  ...
			  {{-- can modify --}}
			  @if($tap->isOrgMaster('shop'))
			  <a class="btn btn-info btn-sm" href="/fashion/edit/{{ $conf['record']->id }}">修改</a>&nbsp<a class="btn btn-danger btn-sm" href="#" data-toggle="modal" data-target="#del">删除</a>
			  	@if($conf['record']->locked)
			  		<a class="btn btn-success btn-sm" href="/fashion/unlock/{{ $conf['record']->id }}">解锁</a>
			  	@else
			  		<a class="btn btn-warning btn-sm" href="/fashion/lock/{{ $conf['record']->id }}">锁定</a>
			  	@endif
			  @endif
			  </caption>
			  ...
			  <tbody>
			    ...
			    <tr>
			      <td></td>
			      <td>{{ $conf['record']->name_en }}</td>
			      
			    </tr>
			    <tr>
			      <td>套系</td>
			      <td>{{ $conf['record']->set }}</td>
			    </tr>
			    <tr>
			      <td></td>
			      <td>{{ $conf['record']->set_en }}</td>
			    </tr>
			    <tr>
			      <td>性别</td>
			...
			@else
				...
				@else
					...
					@if($tap->isOrgMaster('shop'))
				    	<div class="btn-group">
							<a class="btn btn-success btn-sm" href="/goods?fashion={{ $conf['record']->id }}" >显示成品</a>
							<a class="btn btn-default btn-sm" href="/goods/create?fashion={{ $conf['record']->id }}" >+</a>
						</div>

				    @else
				    {{-- nomal --}}
				    <a class="btn btn-success btn-block btn-sm" {!! $conf['record']->locked || $conf['record']->hide ? " disabled=\"disabled\"" : " href=\"/goods?fashion=".$conf['record']->id."\""  !!}>+&nbsp显示成品</a>
				    @endif
    ...
    <div class="modal fade" id="avatar-modal" aria-hidden="true" aria-labelledby="avatar-modal-label" role="dialog" tabindex="-1">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <form class="avatar-form" action="/fashion/image/{{ $conf['record']->id }}" enctype="multipart/form-data" method="post">
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<a href="/fashion/delete/{{ $conf['record']->id }}" class="btn btn-danger">
					删除!
				</a>
			</div>
		</div>

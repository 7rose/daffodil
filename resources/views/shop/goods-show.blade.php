...
<div class="container" id="crop-avatar">
  <div class="col-sm-3">
    ...
    @endif
		...
		<img src="{{ URL::asset($conf['record']->img !='' && $conf['record']->img != null ? $conf['record']->img : 'daffodil/img/default.png') }}"><span class="badge num">21</span>
		</div>
		@else
			<img src="{{ URL::asset($conf['record']->img !='' && $conf['record']->img != null ? $conf['record']->img : 'daffodil/img/default.png') }}"><span class="badge num">21</span>
		@endif
    ...
    <div class="panel panel-default">
	    <div class="panel-body">
	        ...
	        <table class="table table-striped">
			  ...
			  {{-- can modify --}}
			  @if($tap->isOrgMaster('shop'))
			  <a class="btn btn-info btn-sm" href="/goods/edit/{{ $conf['record']->id }}">修改</a>&nbsp<a class="btn btn-danger btn-sm" href="#" data-toggle="modal" data-target="#del">删除</a>
			  	@if($conf['record']->locked)
			  		<a class="btn btn-success btn-sm" href="/goods/unlock/{{ $conf['record']->id }}">解锁</a>
			  	@else
			  		<a class="btn btn-warning btn-sm" href="/goods/lock/{{ $conf['record']->id }}">锁定</a>
			  	@endif
			  @endif
			  </caption>
			  ...
			  <tbody>
			    ...
			    <tr>
			      <td>名称</td>
			      <td>{{ $conf['record']->name }}{{ $conf['record']->name_en }}</td>
			    </tr>
			...
			@else
				...
				@if($conf['record']->img == '' || $conf['record']->img == null)
					<div class="alert alert-warning"><strong>提示:</strong> 图片未设置, 您不可以使用 [进货] 功能,请点击图像进行操作</div>
				@else
					<div class="alert alert-info"><strong>提示:</strong> 您可以使用这个款式! &nbsp&nbsp
					{{-- auth: master or in --}}
					@if($tap->isOrgMaster('shop'))
				    	<div class="btn-group">
							<a class="btn btn-success btn-sm" href="/goods?goods={{ $conf['record']->id }}" >显示成品</a>
							<a class="btn btn-default btn-sm" href="/goods/create?goods={{ $conf['record']->id }}" >+</a>
						</div>

				    @else
				    {{-- nomal --}}
				    <a class="btn btn-success btn-block btn-sm" {!! $conf['record']->locked || $conf['record']->hide ? " disabled=\"disabled\"" : " href=\"/goods?goods=".$conf['record']->id."\""  !!}>+&nbsp显示成品</a>
				    @endif
    ...
    <div class="modal fade" id="avatar-modal" aria-hidden="true" aria-labelledby="avatar-modal-label" role="dialog" tabindex="-1">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <form class="avatar-form" action="/goods/image/{{ $conf['record']->id }}" enctype="multipart/form-data" method="post">
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<a href="/goods/delete/{{ $conf['record']->id }}" class="btn btn-danger">
					删除!
				</a>
			</div>
		</div>
